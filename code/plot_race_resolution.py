...
def analyze_race_resolution(period_blocks, all_blocks_df):
    ...
    for h in potential_contested:
        ...
        # Keep only binary races.  A height with multiple Qubic blocks is a
        # Qubic self fork and does not identify a standard state 0' event.
...
def create_race_resolution_chart(results):
    ...
    ax.set_ylabel('Rate / Probability', fontsize=16)
    # Title removed as requested
    # ax.set_title('Race Resolution Efficiency: Expected vs Observed', fontsize=16)

    ax.set_xticks(x)
    ax.set_xticklabels(df['period'], fontsize=13)
    ax.tick_params(axis='y', labelsize=13)

    ax.legend(fontsize=12)
    ax.grid(True, axis='y', alpha=0.3, linestyle='--')
    # All observed and expected values are below 0.6.  A direct 0--0.65 axis
    # uses the plotting area efficiently without the visual distortion of a
    # broken probability axis.
    ax.set_ylim(0, 0.65)
    ...
    for i in range(len(df)):
        ...
        ax.text(i, max_height + 0.04, f'n={races}', ha='center', va='bottom', fontsize=11)
